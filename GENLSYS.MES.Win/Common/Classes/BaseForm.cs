...
namespace GENLSYS.MES.Win.Common.Classes
{
    public class BaseForm : BaseWCF
    {
        ...
        public void SetFace(Form _form, bool isDoPrivilege)
        {
            ...
            //load icon
            _form.Icon = GENLSYS.MES.Win.Properties.Resources.MES_Logo3;
            #endregion

            //Set resource
            SetControlsFormat(_form);
            SetEnableControlsStyle(_form);
            if (isDoPrivilege)
            {
        ...
        public void SetControlReadOnly(Control _ctrl, bool _readonly)
        {
            if (_ctrl is System.Windows.Forms.TextBox)
            {
                if (_readonly)
                {
                    ((System.Windows.Forms.TextBox)_ctrl).ReadOnly = true;
                    _ctrl.ForeColor = Color.Blue;
                    _ctrl.BackColor = System.Drawing.SystemColors.Control;
                }
                else
                {
                    ((System.Windows.Forms.TextBox)_ctrl).ReadOnly = false;
                    _ctrl.ForeColor = Color.Black;
                    _ctrl.BackColor = Color.White;
                }
            }
        }

        public void SetEnableControlsStyle(Control _parentControl)
        {
            foreach (Control ctrl in _parentControl.Controls)
            {
                if (ctrl is System.Windows.Forms.TextBox)
                {
                    ...
                    if (((System.Windows.Forms.TextBox)ctrl).ReadOnly == true)
                    {
                ...
                {
                    if (((System.Windows.Forms.NumericUpDown)ctrl).ReadOnly == true)
                    {
                        ((System.Windows.Forms.NumericUpDown)ctrl).ReadOnly = false;
                        ctrl.Enabled = false;
                    }
                    if (ctrl.Enabled == false)
                    {
        ...
        public void SetDataGridResource(Infragistics.Win.UltraWinGrid.UltraGrid ctrl)
        {
            ...
            for (int i = 0; i < arrTags.Length; i++)
            {
                ...
                if (tag.rsid!=string.Empty && tag.rsid.Substring(0, 1) == "@")
                    ctrl.DisplayLayout.Bands[0].Columns[i].Header.Caption = tag.rsid.Substring(1, tag.rsid.Length-1);
                else
            ...
            foreach (Control ctrl in _parentControl.Controls)
            {
                if (ctrl.Tag != null)
                {
                    ...
                    if (tag.dbfd != null && tag.dbfd.Trim() != string.Empty)
                    {
                        ...
                        if (prop != null)
                        {
                            SetControlValue(ctrl,tag.dbty,prop.GetValue(obj, null) == null ? "" : prop.GetValue(obj, null).ToString());
                        }
                    }
                }
            ...
            foreach (Control ctrl in _parentControl.Controls)
            {
                if (ctrl.Tag != null)
                {
                    ...
                    if (tag.dbfd != null && tag.dbfd.Trim() != string.Empty)
                    {
                        ...
                        {
                            SetControlValue(ctrl, tag.dbty,ds.Tables[0].Rows[0][tag.dbfd.Trim()] == null ? string.Empty : ds.Tables[0].Rows[0][tag.dbfd.Trim()].ToString());
                        }
                    }
                }
        ...
        private void SetControlValue(Control ctrl,string datatype, string value)
        {
            if (ctrl is System.Windows.Forms.TextBox)
            {
                if (datatype == "")
        ...
        public void ExportExcel(Infragistics.Win.UltraWinGrid.UltraGrid _grid)
        {
            ...
            CreateMessageBox(Public_MessageBox.Information, MessageBoxButtons.OK, null, "导出Excel文件成功.");
        ...
        private void DoPrivilege(string formName, Control _parentControl)
        {
            foreach (Control ctrl in _parentControl.Controls)
            {
                ...
                {
                    string funcurl = formName + Parameter.FUNC_SEPARATOR + ctrl.Name.ToLower();
                    ctrl.Enabled = CheckPrivilege(funcurl);   
                }
        ...
        public List<tsysstaticvalue> GetStaticValue(MES_StaticValue_Type _type)
        {
            ...
            var q = (from p in lstValue
                     where p.svtype == _type.ToString() orderby p.svtext ascending
