    ...
    public class MenuConfig
    {
        ...
        public void CreateMenu(MenuStrip menuStrip, MES_MenuType menuType, EventHandler handler)
        {
            ...
            foreach (tsysmenuconfig menuconfig in menus)
            {
                ...
                {
                    topMenu.Image = (System.Drawing.Image)global::GENLSYS.MES.Win.Properties.Resources.ResourceManager.GetObject(menuconfig.menuicon);
                }
            ...
            foreach (tsysmenuconfig menuconfig in menus)
            {
                ...
                else
                {
                    if (CheckFunctionPrivilege(menuconfig.funcid, Parameter.USER_FUNCTIONS))
                    {
                        ...
                        if (menuconfig.menuicon != null && !menuconfig.menuicon.Trim().Equals(""))
                        {
                            menu.Image = (System.Drawing.Image)global::GENLSYS.MES.Win.Properties.Resources.ResourceManager.GetObject(menuconfig.menuicon);
                        }
                        if (menuconfig.menuitemurl != null && !menuconfig.menuitemurl.Trim().Equals(""))
                        {
        ...
        public static bool IsMDIChildren(object obj)
        {
            bool result = true;
            FieldInfo[] fiArr = obj.GetType().GetFields(BindingFlags.Instance | BindingFlags.GetField | BindingFlags.NonPublic | BindingFlags.ExactBinding);
            foreach (FieldInfo fi in fiArr)
            {
                if (fi.Name.Equals("IsMDI"))
                {
                    if (fi.GetValue(obj).ToString().Equals(MES_Misc.N.ToString()))
                    {
                        result = false;
                        break;
                    }
